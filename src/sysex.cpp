...
std::array<uint8_t, 2> encode14bit(uint16_t value) {
    ...
    return {static_cast<uint8_t>(value & 0x7F), static_cast<uint8_t>(value >> 7)};
}
...
std::array<uint8_t, 4> encode28bit(uint32_t value) {
    if (value > 0x0FFFFFFFu) {
        throw std::runtime_error("value " + std::to_string(value) + " does not fit in 28 bits");
    }
    return {
...
uint32_t decode28bit(uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3) {
    return (static_cast<uint32_t>(b0 & 0x7F)) | (static_cast<uint32_t>(b1 & 0x7F) << 7) |
           (static_cast<uint32_t>(b2 & 0x7F) << 14) | (static_cast<uint32_t>(b3 & 0x7F) << 21);
}
...
std::optional<ParsedResponse> parseResponse(const std::vector<uint8_t>& raw) {
    ...
    resp.payload.assign(raw.begin() + static_cast<long>(idx), raw.end() - 1);

...
inline std::string md5Hex(const std::vector<uint8_t> &input)
{
    ...
    static const uint32_t S[64] = { 7,  12, 17, 22, 7,  12, 17, 22, 7,  12, 17,
    ...
    uint64_t originalLenBits = static_cast<uint64_t>(input.size()) * 8;
    msg.push_back(0x80);
    while (msg.size() % 64 != 56)
        msg.push_back(0x00);
    for (int i = 0; i < 8; ++i)
        msg.push_back(
            static_cast<uint8_t>((originalLenBits >> (8 * i)) & 0xFF));

    for (size_t chunkStart = 0; chunkStart < msg.size(); chunkStart += 64) {
        uint32_t M[16];
        for (int i = 0; i < 16; ++i) {
            M[i] = static_cast<uint32_t>(msg[chunkStart + i * 4])
                   | (static_cast<uint32_t>(msg[chunkStart + i * 4 + 1]) << 8)
                   | (static_cast<uint32_t>(msg[chunkStart + i * 4 + 2]) << 16)
                   | (static_cast<uint32_t>(msg[chunkStart + i * 4 + 3]) << 24);
        }
        ...
        for (uint32_t i = 0; i < 64; ++i) {
            ...
            D = C;
            C = B;
            B = B + md5detail::leftRotate(F, S[i]);
        }
        a0 += A;
        b0 += B;
        c0 += C;
    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            digest[i * 4 + j] =
                static_cast<uint8_t>((vals[i] >> (8 * j)) & 0xFF);
        }
    }

...
void registerRawCommands(CLI::App &app, runner::Context &ctx)
{
    ...
    auto *sub = app.add_subcommand(
        "raw",
        "Send an arbitrary SysEx message - escape hatch for anything not fully wired up, or where the "
        "docs are ambiguous (see the capture command group)");
    sub->add_option(
        "--category", category, "Category byte (hex or decimal), e.g. 0x02");
    sub->add_option(
        "--command", command, "Command byte (hex or decimal), e.g. 0x01");
    sub->add_option(
        "--payload-ascii", payloadAscii, "Payload text, ASCII-encoded");
    sub->add_option("--payload-file",
                    payloadFile,
                    "Read the payload from this file (use - for stdin)");
    sub->add_option(
        "--bytes",
        bytesHex,
    ...
    sub->callback([&ctx] {
        ...
        if (!bytesHex.empty()) {
            ...
            if (category < 0 || category > 0xFF || command < 0
                || command > 0xFF) {
            ...
            if (!payloadFile.empty()) {
                payload =
                    sysex::encodeAscii(runner::readFileOrStdin(payloadFile));
            } else if (!payloadAscii.empty()) {
                payload = sysex::encodeAscii(payloadAscii);
            }
            msg = sysex::buildMessage(static_cast<uint8_t>(category),
        ...
        while (true) {
            ...
            if (!parsed.has_value())
                break; // not a well-formed Electra message; report as-is below
            if (sent.has_value()
                && sysex::isReplyTo(*parsed, sent->category, sent->command))
                break;
            if (!sent.has_value())
                break; // couldn't determine what we sent; report the first reply as-is
        ...
        if (parsed->isAck) {
            std::cout << "ACK";
            if (parsed->transactionId.has_value())
                std::cout << " (txn " << *parsed->transactionId << ")";
            std::cout << "\n";
            runner::exitCode = 0;
            return;
        }
        ...
        runner::writeOutput(
            ctx, runner::formatPayload(ctx, parsed->payloadAsText()));
        runner::exitCode = 0;
    });
}

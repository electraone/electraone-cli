...
void registerControlCommands(CLI::App& app, runner::Context& ctx) {
...

    {
        ...
        auto* sub = control->add_subcommand("update", "Update Control: name/color/visibility/value (runtime)");
        ...
        auto* visibleOpt = sub->add_flag("--visible,!--hidden", visible, "Show or hide the control");
        auto* valueOpt = sub->add_option("--value", value, "New control value");
        sub->callback([&ctx, nameOpt, colorOpt, visibleOpt, valueOpt] {
            if (nameOpt->count() == 0 && colorOpt->count() == 0 && visibleOpt->count() == 0 &&
                valueOpt->count() == 0) {
                throw std::runtime_error("update needs at least one of --name, --color, --visible/--hidden, --value");
            }
            auto idBytes = sysex::encode14bit(static_cast<uint16_t>(id));
            std::vector<uint8_t> params{idBytes[0], idBytes[1]};

            JsonDocument doc;
            if (nameOpt->count() > 0) doc["name"] = name;
            if (colorOpt->count() > 0) doc["color"] = color;
            if (visibleOpt->count() > 0) doc["visible"] = visible;
            if (valueOpt->count() > 0) doc["value"] = value;
    ...
    }
    {
        ...
        auto* sub = control->add_subcommand("override-text", "Override Value Text (runtime, max 15 characters)");
        sub->add_option("--id", id, "Control ID")->required();
        sub->add_option("--value-id", valueId, "Value ID (0x00-0x08)")->required();
        sub->add_option("--text", text, "Override text")->required();
        sub->callback([&ctx] {
            if (text.size() > 15) {
                throw std::runtime_error("override text must be at most 15 characters");
            }
            auto idBytes = sysex::encode14bit(static_cast<uint16_t>(id));

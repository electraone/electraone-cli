...
namespace commands
{
    ...
    inline uint8_t parsePortSelector(const std::string &value)
    {
        ...
        if (v == "ctrl")
            return 0x02;
        throw std::runtime_error("invalid port selector '" + value
    ...
    {
        auto *b = sub->add_option("-b,--bank", bank, "Bank number (0-6)")
                      ->default_val(0);
        auto *s = sub->add_option("-s,--slot", slot, "Slot number (0-11)")
                      ->default_val(0);
        if (required) {
            b->required();
            s->required();
    ...
    inline std::vector<uint8_t> optionalBankSlotParams(CLI::Option *bankOpt,
                                                       ...
                                                       int slot)
    {
        if (bankOpt->count() > 0 || slotOpt->count() > 0)
            return bankSlotParams(bank, slot);
        return {};
    }
                                  ...
                                  bool &all)
    {
        ...
        sub->add_flag("--control-set",
                      controlSet,
                      "Subscribe to Control Set Switch events");
        sub->add_flag(
            "--usb-host", usbHost, "Subscribe to USB Host Change events");
    ...
    inline void addGlobalOptions(CLI::App &app, runner::Context &ctx)
    {
        ...
        app.add_option(
            "--txn-id",
            ctx.txnId,
            "Optional 14-bit transaction ID to attach to the request");
        ...
        app.add_flag(
            "--raw",
            ctx.raw,
            "Print/write the raw response bytes as hex instead of decoding");
        app.add_flag(
            "--pretty",
            ctx.pretty,

...
namespace commands
{
    ...
    namespace
    {
        ...
        void renderTreeArrayValue(std::ostream &os,
                                  ...
                                  int indent)
        {
            ...
            for (JsonVariantConst item : arr) {
                if (!item.is<JsonObjectConst>())
                    allObjects = false;
                if (item.is<JsonObjectConst>() || item.is<JsonArrayConst>())
                    allScalar = false;
            }
            ...
            if (value.is<JsonObjectConst>()) {
                for (JsonPairConst kv : value.as<JsonObjectConst>()) {
                    ...
                    if (v.is<JsonObjectConst>()) {
                        ...
                        renderTreeArrayValue(
                            os, key, v.as<JsonArrayConst>(), indent);
                    } else {
        ...
        void renderGrid(std::ostream &os, JsonArrayConst arr, int indent)
        {
            ...
            size_t available = terminalWidth() > pad.size()
                                   ? terminalWidth() - pad.size()
                                   : terminalWidth();
            size_t numCols =
                std::max<size_t>(1, available / std::max<size_t>(1, colWidth));
            ...
            for (size_t r = 0; r < numRows; ++r) {
                ...
                for (size_t c = 0; c < numCols; ++c) {
                    ...
                    if (idx < items.size()) {
                        bool lastCol = (c + 1 == numCols)
                                       || (idx + numRows >= items.size());
                        if (lastCol) {
                            os << items[idx];
                        } else {
                            os << std::left
                               << std::setw(static_cast<int>(colWidth))
                               << items[idx];
                        }
                    }
                }

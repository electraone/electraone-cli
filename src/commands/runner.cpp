...
std::optional<sysex::ParsedResponse> sendAndReceive(const Context& ctx, uint8_t category,
                                                      uint8_t command,
...
void runQuery(const Context& ctx, uint8_t category, uint8_t command,
    ...
    if (!resp.has_value()) {
        std::cerr << "error: timed out waiting for a reply from the device\n";
        exitCode = 2;
        return;
    }
    if (ctx.raw) {
    ...
    if (resp->isNack) {
        std::cerr << "NACK";
        if (resp->transactionId.has_value()) std::cerr << " (txn " << *resp->transactionId << ")";
        std::cerr << "\n";
        exitCode = 1;
        return;
    }
...
void runAction(const Context& ctx, uint8_t category, uint8_t command,
    ...
    if (!resp->isAck) {
        ...
        std::cout << "OK (unexpected reply category=0x" << std::hex << int(resp->category)
                   << " command=0x" << int(resp->command) << std::dec << ")\n";
        if (!resp->payload.empty()) std::cout << formatPayload(ctx, resp->payloadAsText()) << "\n";
        exitCode = 0;
        return;
    }
    std::cout << "OK";
    if (resp->transactionId.has_value()) std::cout << " (txn " << *resp->transactionId << ")";
    std::cout << "\n";
    exitCode = 0;
}
...
std::string formatPayload(const Context& ctx, const std::string& text) {
    if (ctx.human) return commands::formatHumanReadable(text);
    if (!ctx.pretty) return text;
    JsonDocument doc;
...
void writeOutput(const Context& ctx, const std::string& text) {
    ...
    if (!f) {
        throw std::runtime_error("could not open output file: " + ctx.outputFile);
    }
    f << text;
}

...
Response makeResponse(uint8_t category, uint8_t command, std::vector<uint8_t> payload = {}) {
    Response r;
    r.category = category;
    r.command = command;
...
TEST_CASE("describeEvent: Preset Switch") {
    CHECK(describeEvent(makeResponse(0x7E, 0x02, {2, 5})) == "Preset Switch: bank=2 slot=5");
}
...
TEST_CASE("describeEvent: Pot Touch decodes the 14-bit control id") {
    ...
    CHECK(describeEvent(makeResponse(0x7E, 0x0A, {3, 18, 8, 1})) == "Pot Touch: pot=3 control=1042 touched=1");
}

TEST_CASE("describeEvent: Page Switch and Control Set Switch") {
    CHECK(describeEvent(makeResponse(0x7E, 0x06, {4})) == "Page Switch: page=4");
    CHECK(describeEvent(makeResponse(0x7E, 0x07, {2})) == "Control Set Switch: set=2");
}

TEST_CASE("describeEvent: disambiguates the documented 0x7E/0x08 byte collision by payload length") {
    ...
    CHECK(describeEvent(makeResponse(0x7E, 0x08, {6})) == "Preset Bank Switch: bank=6");
}

TEST_CASE("describeEvent: Snapshot Bank Switch") {
    CHECK(describeEvent(makeResponse(0x7E, 0x04, {1})) == "Snapshot Bank Switch: bank=1");
}
...
TEST_CASE("describeEvent: Progress Report decodes the 28-bit byte count") {
    ...
    CHECK(describeEvent(makeResponse(0x7E, 0x2D, {0x7F, 0x7F, 0x7F, 0x07})) ==
          "Progress Report: " + std::to_string(sysex::decode28bit(0x7F, 0x7F, 0x7F, 0x07)) + " bytes transferred");
}

TEST_CASE("describeEvent: Log Message") {
    std::string text = "[1234] hello";
    Response r = makeResponse(0x7F, 0x00, std::vector<uint8_t>(text.begin(), text.end()));
    CHECK(describeEvent(r) == "Log: " + text);
}

TEST_CASE("describeEvent: MIDI Learn Info reassembles the JSON that the single-byte header split") {
    ...
    Response r = makeResponse(0x03, static_cast<uint8_t>(json[0]),
...
TEST_CASE("describeEvent: unknown category/command falls back to a hex dump") {
    CHECK(describeEvent(makeResponse(0x11, 0x22, {0xAA, 0xBB})) == "0x11/0x22 payload=AA BB");
}

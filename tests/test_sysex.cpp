...
{
    for (uint16_t v : { static_cast<uint16_t>(0),
...
TEST_CASE("buildMessage: with params")
{
    auto msg = buildMessage(
        0x05, 0x01, { 0x02, 0x03 }); // Remove Preset, bank=2 slot=3
    std::vector<uint8_t> expected = { 0xF0, 0x00, 0x21, 0x45, 0x05,
                                      0x01, 0x02, 0x03, 0xF7 };
    CHECK(msg == expected);
}

TEST_CASE(
    "buildMessage: optional transaction id is emitted as 00 lsb msb right after the manufacturer id")
{
    auto msg = buildMessage(0x02, 0x7F, {}, 1042);
    std::vector<uint8_t> expected = { 0xF0, 0x00, 0x21, 0x45, 0x00,
                                      18,   8,    0x02, 0x7F, 0xF7 };
    CHECK(msg == expected);
}
...
TEST_CASE("parseResponse: decodes a NACK with an echoed transaction id")
{
    std::vector<uint8_t> raw = {
        0xF0, 0x00, 0x21, 0x45, 0x7E, 0x00, 18, 8, 0xF7
    };
    auto resp = parseResponse(raw);
    REQUIRE(resp.has_value());
    CHECK(resp->isNack);
...
TEST_CASE(
    "parseResponse: decodes the leading transaction-id prefix on a data reply")
{
    std::vector<uint8_t> raw = { 0xF0, 0x00, 0x21, 0x45, 0x00, 18,
                                 8,    0x02, 0x7F, 'O',  'K',  0xF7 };
    auto resp = parseResponse(raw);
    REQUIRE(resp.has_value());
    REQUIRE(resp->transactionId.has_value());
    CHECK(*resp->transactionId == 1042);
...
TEST_CASE("parseResponse: rejects malformed messages")
{
    ...
    CHECK_FALSE(parseResponse({ 0xF0, 0x00, 0x21, 0x45, 0x02, 0x7F })
                    .has_value()); // missing F7
    CHECK_FALSE(parseResponse({ 0x00, 0x21, 0x45, 0x02, 0x7F, 0xF7 })
                    .has_value()); // missing F0
    CHECK_FALSE(parseResponse({ 0xF0, 0x00, 0x21, 0x46, 0x02, 0x7F, 0xF7 })
                    .has_value()); // wrong mfr id
    CHECK_FALSE(parseResponse({ 0xF0, 0x00, 0x21, 0x45, 0xF7 })
                    .has_value()); // too short
}
...
TEST_CASE(
    "isReplyTo: a query reply comes back as category 0x01, not an echo of the request's 0x02")
{
...
TEST_CASE(
    "isReplyTo: an unrelated unsolicited event is not mistaken for the reply")
{
...
TEST_CASE(
    "isReplyTo: non-query requests still require an exact category/command echo")
{

...
namespace electraone
{
    ...
    public:
        ...
        std::optional<Response> switchPresetSlot(int bank,
                                                 int slot); // runtime, volatile
        std::optional<Response> setPresetSlot(int bank,
                                              int slot); // runtime, volatile
        std::optional<Response> reloadPresetSlot();
        std::optional<Response>
            loadPreloadedPreset(int bank,
                                int slot,
                                const std::string &path); // runtime, volatile
        ...
        std::optional<Response> executeLuaCommand(
            const std::string &code); // runtime, max 65535 bytes
        ...
        std::optional<Response> swapSnapshots(const std::string &projectId,
                                              ...
                                              int toBank,
                                              int toSlot);
        std::optional<Response> setSnapshotSlot(const std::string &projectId,
                                                int bank,
                                                int slot); // runtime, volatile
        ...
        std::optional<Response> swapCaptures(const std::string &projectId,
                                             ...
                                             int toBank,
                                             int toSlot);
        std::optional<Response> setCaptureSlot(const std::string &projectId,
                                               int bank,
                                               int slot); // runtime, volatile
        ...
        std::optional<Response>
            overrideValueText(int id,
                              int valueId,
                              const std::string &text); // max 15 chars

        // ---- UI (runtime) ----
        std::optional<Response> switchPage(int page);
        std::optional<Response> switchControlSet(int set);
        std::optional<Response>
            setBottomBarText(const std::string &text); // max 40 chars

...
namespace sysex
{
    ...
    // Splits a 28-bit value into four 7-bit bytes (least-significant first),
    // the scheme the File Transfer API uses for file sizes (Register Files)
    // and byte counts (Report Progress).
    std::array<uint8_t, 4> encode28bit(uint32_t value);

    // Recombines a 28-bit value from four 7-bit bytes (least-significant
    // first).
    ...
    // character is >= 0x80 (Electra SysEx payloads are restricted to 7-bit
    // bytes).
    std::vector<uint8_t> encodeAscii(const std::string &text);

    // Builds a full F0..F7 message: F0 00 21 45 [00 id-lsb id-msb] category
    //   command params... F7
    // txnId is optional; when present it is emitted as the 3-byte
    //   transaction-id
    // prefix immediately after the manufacturer ID, per the documented
    //   convention.
    std::vector<uint8_t>
    ...
    // Renders a byte vector as space-separated uppercase hex,
    // e.g. "F0 00 21 45".
